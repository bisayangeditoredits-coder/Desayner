...
import { Search, X, Loader2, ImageIcon, ChevronDown } from 'lucide-react';
...
export default function StockPhotosPage() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => doSearch(query, 1, orientation), 600);
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !loading && !loadingMore && results.length > 0 && page < totalPages) {
          doSearch(query, page + 1, orientation);
        }
      },
      { threshold: 0.1 }
    );
  ...
  }, [loading, loadingMore, results.length, page, totalPages, query, orientation, doSearch]);
          ...
          <div ref={loadMoreRef} style={{ textAlign: 'center', marginTop: '2rem', paddingBottom: '2rem' }}>
            {loadingMore && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', color: '#6e6d7a', fontWeight: 600 }}>
                <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> Loading more photos...
              </div>
            )}
          </div>
        )}
